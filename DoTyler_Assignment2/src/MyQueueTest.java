...
public class MyQueueTest {
	...
	public double a2 = 12, b2 = 54, c2 = 100.2, d2 = -75, e2 = 5, f2 = 0;
	...
	public void testDequeueStudent() {
		...
		try {
			assertEquals(a2, doubleQ.dequeue());
			assertEquals(b2, doubleQ.dequeue());
			assertEquals(c2, doubleQ.dequeue());
			//Queue is empty, next statement should cause QueueUnderFlowException
			doubleQ.dequeue();
			assertTrue(false, "Error: Enqueue method was called on a full queue");
		}
		catch (QueueUnderflowException e){
			assertTrue(true, "Error: Enqueue method was called on a full queue");
		}
		catch (Exception e){
			assertTrue(false, "Error: Enqueue method was called on a full queue");
		}
	...
	public void testEnqueueStudent() {
		...
		try {
			...
			assertEquals(true, doubleQ.enqueue(d2));
			assertEquals(4, doubleQ.size());
			assertEquals(true, doubleQ.enqueue(e2));
	...
	public void testToStringStudent() {
		//Use the doubleQ for student tests
		//fail("Not yet implemented");
		assertEquals("12.054.0100.2", doubleQ.toString());
		try {
			doubleQ.enqueue(d2);
		} catch (QueueOverflowException e) {
		...
		assertEquals("12.054.0100.2-75.0", doubleQ.toString());
		try {
			doubleQ.enqueue(e2);
		} catch (QueueOverflowException e) {
			// TODO Auto-generated catch block
		...
		assertEquals("12.054.0100.2-75.05.0", doubleQ.toString());
	}
